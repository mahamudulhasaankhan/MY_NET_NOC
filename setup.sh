...
if [[ ! -f /etc/nms/nms.env ]]; then
    # Per-installation secrets are generated at setup time — NEVER hardcoded
...
if [[ ! -f /etc/nginx/.htpasswd ]]; then
    ...
    printf "admin:%s\n" "$(openssl passwd -apr1 "${GEN_ADMIN_PASS}")" > /etc/nginx/.htpasswd
...
cp "${INSTALL_DIR}/infra/deploy/nginx/nms_security_headers.conf" /etc/nginx/nms_security_headers.conf 2>/dev/null || true
...
if [[ -n "${GEN_ADMIN_PASS}" ]]; then
    echo -e " 🔑 Nginx admin password (basic auth): ${CYAN}${GEN_ADMIN_PASS}${NC}"
    echo -e "    ${YELLOW}⚠️  Save it now and change it after first login!${NC}"
fi
if [[ -n "${GEN_ENCRYPTION_KEY}" ]]; then
    echo -e " 🔐 Data encryption key: ${CYAN}generated & stored in /etc/nms/nms.env${NC}"
fi
echo ""
