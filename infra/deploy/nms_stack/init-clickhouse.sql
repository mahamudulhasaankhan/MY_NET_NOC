...
-- Monitoring user provisioning: see apply_secrets.sh
...
CREATE TABLE IF NOT EXISTS default.netflow_raw (
    ...
    src_port UInt16, dst_port UInt16, packets UInt64, exporter_ip String,
    id UInt64 DEFAULT cityHash64(toString(timestamp), src_ip, dst_ip, toString(bytes), protocol,
                                 toString(src_port), toString(dst_port), toString(packets), exporter_ip)
