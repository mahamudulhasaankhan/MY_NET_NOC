...
| Layer | Nodes | Access |
|---|---|---|
| Postgres | pg-0 (primary) / pg-1 (standby, streaming, lag ~0) | **front door** 127.0.0.1:6432 (pgpool-proxy, HA) — direct 15432/25432 exist but apps no longer use them |
...
| Metrics bridge | nginx on 172.17.0.1:8000 — serves ONLY `/metrics` to Docker containers (VictoriaMetrics scrape); every other path 403. Do NOT replace with a raw socat forwarder or UFW-open port 8000 to the LAN. | containers only (UFW 172.16/12) |
...
## Metrics pipeline
Engine `/metrics` binds loopback only; nginx metrics bridges (docker0 → 127.0.0.1, `/metrics` only, everything else 403) expose one bridge per Tier-B instance — 172.17.0.1:18080→:8000 … :18084→:8004 — so VictoriaMetrics scrapes all 5 instances with an `instance` label (the NMS **System Health** page at `/monitoring/system-health`, backed by `GET /api/v3/system/health/metrics`, renders these live). If the engine `up` drops to 0: check the bridges (`ss -ltn | grep 1808`) and the System Health page's Engine section.

**ClickHouse viewer (System Health page):** `GET /api/v3/system/netflow` and `GET /api/v3/system/syslog` query CH over HTTP 8123 with the same `CLICKHOUSE_PASSWORD`/`NMS_CLICKHOUSE_PASSWORD` auth the engine writer uses. If the NetFlow/Syslog cards show "ClickHouse unavailable", confirm the container is up (`docker ps | grep clickhouse`) and that `CLICKHOUSE_PASSWORD` is present in `/etc/nms/nms.env` (the API engine instance reads it via `EnvironmentFile`).

## Alert delivery (telegram-relay)
...
- Alerters: `nms_watchdog.service` (1-min timer: auto-heal engines/nginx/pgpool + pgpool node re-attach + HA sweep, TG on FAIL→OK transitions, creds via DB `settings` fallback) and `nms-ha-daily.timer` (07:30 daily: verify-ha + backup freshness >36h → TG via relay) and `verify-ha.sh` container crash-loop checks. All are safe to re-run manually (`systemctl start`).
...
4. The stack containers (PG/pgpool, Redis+Sentinel, CH, NATS, freeradius, …) come up via `infra/deploy/db_cluster/` + `infra/deploy/nms_stack/` compose files; NATS/CH credentials must match nms.env.
...
## VMware-level VM snapshot (hypervisor DR, single-host strategy)

The NOC runs as a VMware VM (`sda` = VMware Virtual disk). The app-level backups above protect DATA; a hypervisor snapshot protects the whole MACHINE (OS + docker + config + data) and is the fastest full-recovery path after host-level damage. Run from the VMware host (ESXi/vCenter):

```bash
# 1. Freeze the DB briefly so the snapshot is crash-consistent, not torn:
ssh <noc> 'sudo docker stop pgpool-proxy pg-pgpool pg-pgpool-2'   # ~5s, suppresses failover
# 2. Snapshot (vCenter UI: Snapshot -> Take Snapshot, no memory; or):
vim-cmd vmsvc/snapshot.create <vmid> nms-daily "nightly DR point" 0 0
# 3. Unfreeze immediately:
ssh <noc> 'sudo docker start pg-pgpool pg-pgpool-2 pgpool-proxy'
# 4. Verify: sudo bash /usr/local/bin/verify-ha.sh   (ALL CHECKS PASSED)
```

Rules: keep **max 2-3 snapshot chains** (snapshots are deltas — they grow and slow the datastore); delete snapshot chains older than 3 days; for long-term retention export the VM (OVA) monthly to separate storage instead of keeping old snapshots. Restore = revert snapshot (minutes) then `verify-ha.sh`; data rollback to the snapshot point, newer data recoverable from the nightly Telegram/R2 backup.

## R2 offsite sync (optional second offsite copy)

Nightly backups already go to Telegram (encrypted). For a proper archive bucket: create a Cloudflare R2 bucket + API token, then on the NOC:

```bash
sudo apt install -y rclone
rclone config   # new remote "r2", type s3, endpoint https://<accountid>.r2.cloudflarestorage.com
# then add ONE line to /etc/nms/nms.env:
echo 'R2_REMOTE=r2:nms-backups' | sudo tee -a /etc/nms/nms.env
```

The next nightly run (or `sudo systemctl start nms_backup.service`) picks it up automatically — backup.sh syncs the last 48h of archives and Telegram-notifies on success/failure. Set a bucket lifecycle rule (expire >30d) on the R2 side.
