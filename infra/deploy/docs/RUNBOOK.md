...
## Metrics pipeline
Engine `/metrics` binds loopback only; `nms_metrics_bridge.service` (systemd, socat) forwards docker0:8000 → 127.0.0.1:8000 so VictoriaMetrics can scrape (the NMS **System Health** page at `/monitoring/system-health`, backed by `GET /api/v3/system/health/metrics`, renders these live). If the engine `up` drops to 0: check the bridge unit (`systemctl status nms_metrics_bridge`) and the System Health page's Engine section.
