...
cp -f "$NMS_REPO/infra/deploy/nginx/nms_security_headers.conf" /etc/nginx/nms_security_headers.conf
...
# nginx.conf only includes /etc/nginx/sites-enabled/*. Rendering into
# sites-available and stopping there changes NOTHING at runtime — that is how
# the live config silently drifted from this template (stale X-Forwarded-For,
# no WAF JSON 403 handler, no CSP). Keep sites-enabled a symlink to the file we
# just rendered so a re-render can never become a no-op again.
ln -sfn ../sites-available/nms_proxy /etc/nginx/sites-enabled/nms_proxy
