...
wait_healthy() {
    ...
    for i in $(seq 1 60); do
        if curl -sf --max-time 2 -A "nms-deploy-healthcheck" "http://127.0.0.1:${port}/api/v2/health" > /dev/null 2>&1; then
...
curl -sf -k -A "nms-deploy-healthcheck" https://127.0.0.1/api/v2/health > /dev/null || rollback "nginx health check failed"
