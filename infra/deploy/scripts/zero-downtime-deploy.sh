...
for i in "${!WEB_INSTANCES[@]}"; do
    ...
    # Migration owner: exactly one instance (first port) runs schema
    # migrations via NMS_RUN_MIGRATIONS=1. Previously the Go code anchored on
    # PORT==8000, which silently skipped migrations if :8000 was down.
    if [ "$port" = "${WEB_INSTANCES[0]}" ]; then
        if ! sudo grep -q '^NMS_RUN_MIGRATIONS=' "$env_file" 2>/dev/null; then
            printf 'NMS_RUN_MIGRATIONS=1\n' | sudo tee -a "$env_file" > /dev/null
            log "  marked $env_file as migration owner (NMS_RUN_MIGRATIONS=1)"
        fi
    fi
done
