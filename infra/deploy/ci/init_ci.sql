...
-- Dedicated operator for the Playwright journey/login-flow specs. 'shihab' is a
-- super-user, so mandatory-2FA enforcement hands it a LIMITED enrollment session
-- (redirect /settings/account-security?enroll=1) and every non-2FA API answers
-- 403 — the specs need a real, non-privileged session (redirect /dashboard).
-- Same bcrypt hash as 'shihab', so it shares the NMS_TEST_PASSWORD secret.
INSERT INTO users (username, password_hash, two_factor_enabled, name, role, access_level, idle_timeout_minutes)
VALUES ('e2e_operator', '$2y$10$PfmBkfnZ84rcD2cx.4XaN.RAjYG7aM7inNOWnzifZjKmioM1MFbzS', FALSE, 'E2E Operator', 'operator', 'full', 480)
ON CONFLICT (username) DO NOTHING;
