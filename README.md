...
Powered by an ultra-low latency **Go Core Engine** running in an **8-Instance Split-Role Architecture** (5 Active-Active Web API Engines + 3 Leader-Elected Polling Workers) and a dynamic **React Single Page Interface**, the platform aggregates and analyzes **9 distinct network telemetry pipelines**—ranging from microsecond-level SNMP counters and **100,000+ flows/sec NetFlow/sFlow** ingestion to autonomous DDoS BGP blackholing, AI-driven anomaly detection, FreeRADIUS subscriber accounting, and Git-versioned router configuration backups. Powered by **31 background scheduler tasks** across 6 task groups running on the leader-elected poller cluster.
...
To ensure full transparency into how the platform processes and routes every data type, the table below breaks down the 9 unified data pipelines:
...
| **4** | **DDoS Mitigation** | Manual trigger & policy (Security Center) | DDoS Engine + MikroTik RouterOS API | **PostgreSQL / MikroTik** | One-click BGP `/32` blackhole dispatch & release, per-router mitigation policies & drill simulation. |
...
| **9** | **AI Anomaly & Intelligence** | Historical metrics, flow patterns, security events | Go Scheduler (`anomaly_detection`, `ddos_intelligence`, `ai_forecaster`) | **PostgreSQL `ai_findings` + VictoriaMetrics** | ML-driven anomaly scoring, predictive capacity forecasting, DDoS intelligence, and AI Inspector for root-cause diagnosis. |
...
- 🤖 **AI-Driven Intelligence**: 31 background scheduler tasks — including anomaly detection, DDoS intelligence, predictive capacity forecasting, and AI Inspector — all running on the leader-elected poller cluster.
