...
## 5. DDoS Protection & Mitigation

- Open **Security > DDoS Protection**.
- **BGP Blackholing**:
  - When an attack is detected on a specific target IP (e.g. `103.x.x.x`), operators can inject a `/32` blackhole route via BGP community with a single click to discard traffic at the upstream border.
...
- **Health Endpoint**: Test platform status via terminal:
  ```bash
  curl -k https://<server-ip>/api/v3/health
  ```
