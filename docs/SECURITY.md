...
│    • Opaque server-side session tokens (Redis-backed) with instant revocation, IP+UA binding & step-up re-auth    │
├─────────────────────────────────────────────────────────────────────────┤
...
│    • MikroTik RouterOS API-driven DDoS blackhole & firewall push        │
│    • BGP /32 blackhole dispatches to upstream transit                   │
...
- **Volumetric Flood Defense**: Per-router DDoS policies drive MikroTik BGP community-based `/32` blackholing — triggered manually or per policy from the Security Center — to protect upstream transit links.
