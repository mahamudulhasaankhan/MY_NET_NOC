...
## 3. The 9 Unified Data Pipelines Explained
...
### 4. DDoS Mitigation Pipeline
- **Input Data**: Attack observations from NOC monitoring and upstream provider alerts.
- **Ingestion**: One-click blackhole / release from the Security Center against per-router `ddos_router_policies`.
- **Action**: MikroTik RouterOS API injects a BGP `/32` blackhole route to upstream transits and notifies the NOC on-call team via Telegram.
...
### 9. AI Anomaly Detection & Intelligence Pipeline
- **Input Data**: Historical VictoriaMetrics time-series (CPU, bandwidth, PPPoE), ClickHouse flow patterns, PostgreSQL security events and audit logs.
- **Ingestion**: Go built-in scheduler tasks running on the leader-elected poller — `anomaly_detection` (every 5 min), `ddos_intelligence` (periodic), `ai_forecaster` (daily), `predictive_maintenance` (4h), `security_posture` (periodic), `capacity_forecast` (periodic).
- **Storage**: **PostgreSQL `ai_findings` table** — structured findings with agent name, severity, router IP, category, title, description, and raw JSON evidence.
- **Output**: AI Inspector root-cause diagnosis, capacity forecasting alerts, DDoS intelligence scoring, security posture reports — all surfaced in the NOC dashboard and dispatched via Telegram for critical findings.
